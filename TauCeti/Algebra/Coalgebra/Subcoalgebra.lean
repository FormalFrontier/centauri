...
`Subcoalgebra.IsFinite` to state that each vector lies in a finite-dimensional subcoalgebra.

## Main definitions

* `TauCeti.Subcoalgebra`: a submodule stable under comultiplication.
* `TauCeti.Subcoalgebra.IsFinite`: finite generation of the underlying submodule.
* `TauCeti.Subcoalgebra.groupLikeSpan`: the one-dimensional subcoalgebra generated by a
  group-like element.
...
instance : SetLike (Subcoalgebra R C) C where
  ...
  coe_injective' D E h := by
    ...
    cases E with
    | mk carrier' hE =>
    change (carrier : Set C) = carrier' at h
    have hcarrier : carrier = carrier' := SetLike.ext' h
    cases hcarrier
    rfl
...
/-- A subcoalgebra is finite when its underlying submodule is finitely generated. -/
def IsFinite (D : Subcoalgebra R C) : Prop :=
  Module.Finite R D.toSubmodule

/-- Being finite as a subcoalgebra is finite generation of the underlying submodule. -/
theorem isFinite_iff (D : Subcoalgebra R C) :
    D.IsFinite ↔ Module.Finite R D.toSubmodule :=
  Iff.rfl
